<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('admin.barang.update', $barang->id) }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
                @csrf
                @method('PUT') <h3 class="text-lg font-semibold mb-6">Form Edit Barang</h3>
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_barang" id="nama_barang" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('nama_barang', $barang->nama_barang) }}" required>
                    </div>
                    <div>
                        ...
                        <select id="kategori_id" name="kategori_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            ...
                            @foreach ($kategoris as $kategori)
                                <option value="{{ $kategori->id }}" @if(old('kategori_id', $barang->kategori_id) == $kategori->id) selected @endif>{{ $kategori->nama_kategori }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="satuan" class="block mb-2 text-sm font-medium text-gray-900">Satuan *</label>
                        <input type="text" name="satuan" id="satuan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500" value="{{ old('satuan', $barang->satuan) }}" required>
                    </div>
                </div>

                <div class="mt-6">
                    <label for="stok" class="block mb-2 text-sm font-medium text-gray-900">Stok *</label>
                    <input type="number" name="stok" id="stok" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ old('stok', $barang->stok) }}" required>
                </div>
